...
async function deleteFragment(ownerId, id) {
  ...
  try {
    await Promise.all([s3Client.send(command), ddbDocClient.send(dynamo_command)]);
  } catch (err) {
    const { Bucket, Key } = params;
    const { TableName } = dynamo_params;
    logger.error({ err, Bucket, Key, TableName }, 'Error deleting fragment data from S3');
